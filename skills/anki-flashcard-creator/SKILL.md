...
**Format the acceptable-answers list as a bulleted `<ul>`, not a run of semicolons or commas.** A free-recall back holds several distinct points, and the learner has to scan it against what they just produced - stacked bullets make that scan instant, whereas `point A; point B; point C; point D` on one line is exactly the wall-of-text the rest of this skill avoids. Put the `<ul>` in the **answer** slot (with a short lead-in) and the self-grade rule in the **context** slot, so the back still reads answer-first. This is the one place `build_deck.py` permits an HTML list in a basic answer: it allows `<ol>/<ul>/<li>` in the answer of a card tagged `enumeration`, and blocks it everywhere else. So the bullets only work if the card actually carries that tag - untagged, the build hard-fails (correctly, because on any normal card a list is the defect the guard exists to catch). Shape:

```json
{"type": "basic",
 "front": "📊 <i>(Exam recall)</i> Give two or more conditions that make <b>price skimming</b> a suitable strategy.",
 "answer": "Any two or more of:<ul><li>an innovative, new-and-different product</li><li>a strong, well-known brand</li><li>early adopters willing to pay a premium</li><li>limited competition at launch</li></ul>",
 "context": "<i>Self-grade: Again if fewer than 2, Good if 2+ with a clear reason each.</i>",
 "tags": ["business::theme1::pricing_strategies", "enumeration"]}
```

Use `<ol>` instead of `<ul>` only when the points are expected in a set order (e.g. the stages of a process); for an unordered "give any N" set, `<ul>` is right.
